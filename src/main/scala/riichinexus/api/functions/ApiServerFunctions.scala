...
import riichinexus.api.{ApiRuntimeContext, ApiServerConfig, ApiServerHandle, ApiServerState}

object ApiServerFunctions:

  def state(runtime: ApiRuntimeContext, config: ApiServerConfig): ApiServerState =
    ApiServerState(
      runtime = runtime,
      config = config,
