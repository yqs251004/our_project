...
import riichinexus.microservices.tournament.api.competition.`private`.RecordClubTournamentDeclinePrivateAPIMessage
...
final case class DeclineClubTournamentAPIMessage(
  ...
  override def plan(context: ApiPlanContext): IO[TournamentMutationView] =
    for
      ...
      _ <- declineTournament(context, requestedClubId, requestedTournamentId, actor)
  ...
  private def declineTournament(
    ...
    for
      ...
      _ <- RecordClubTournamentDeclinePrivateAPIMessage(tournamentId, clubId).plan(context)
    yield ()
