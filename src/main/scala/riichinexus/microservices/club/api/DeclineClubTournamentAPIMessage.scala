      ...
      tournamentModule.transactionManager.inTransaction {
        ...
        tournamentModule.tournamentRepository.findById(tournamentIdValue).foreach { tournament =>
          val trackedParticipation =
            tournament.participatingClubs.contains(clubIdValue) || tournament.whitelist.exists(_.clubId.contains(clubIdValue))
          if !trackedParticipation then
            throw IllegalArgumentException(
              s"Club ${clubIdValue.value} is not participating in tournament ${tournamentIdValue.value}"
            )
          tournamentModule.tournamentRepository.save(tournament.removeClub(clubIdValue))
        }
      }
