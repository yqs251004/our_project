...
import riichinexus.microservices.club.objects.relation.{ClubRelationKind, ClubRelationView}
import riichinexus.microservices.club.tables.clubs.ClubTable
import riichinexus.microservices.player.objects.`private`.PlayerPrivateView
import riichinexus.microservices.player.objects.PlayerStatus
import riichinexus.microservices.club.objects.profile.PublicClubDirectoryEntry
...
final case class ListPublicClubsAPIMessage(
  ...
  override def plan(context: ApiPlanContext): IO[PagedResponse[PublicClubDirectoryEntry]] =
    for
      ...
      appliedFilters = publicClubDirectoryFilters
      ...
      filteredEntries = filterPublicClubDirectoryEntries(entries, nameFilter, relation)
  ...
  private def publicClubDirectoryFilters: Map[String, String] =
    Vector(
      name.filter(_.nonEmpty).map(QueryFilterField.toString(QueryFilterField.Name) -> _),
      relation.map(value => QueryFilterField.toString(QueryFilterField.Relation) -> ClubRelationKind.toString(value))
    ).flatten.toMap
  ...
  private def filterPublicClubDirectoryEntries(
    ...
    entries
      .filter(club => name.forall(riichinexus.system.TextSearch.containsIgnoreCase(club.name, _)))
      .filter(club => relation.forall(relationKind => club.relations.exists(_.relation == relationKind)))
      .sortBy(_.name)
