...
final class TournamentStageCompletionCoordinator(
  ...
  def completeStage(
    ...
    TournamentTable.findById(connection, command.tournamentId).map { tournament =>
      ...
      val stageTables = TournamentGameTable.findByTournamentAndStage(connection, command.tournamentId, command.stageId)
      ensureStageCanComplete(connection, tournament, stage, stageTables, command.completedAt)

      val advancement =
        TournamentStageQueries.stageAdvancementPreview(connection, command.tournamentId, command.stageId, command.completedAt)

      TournamentTable.save(connection, tournament.updateStage(command.stageId, _.complete))
      advancement
    }
