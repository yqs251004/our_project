...
private[tournament] object TournamentSettlementCoordinator:
  ...
  private def buildSettlementEntries(
    ...
    resolvedPlayers.distinct.foldLeft(IO.pure(Map.empty[PlayerId, Vector[ClubId]])) { (previous, playerId) =>
      previous.flatMap(resolved =>
        ResolvePlayerBoundClubIdsPrivateAPIMessage(playerId)
          .plan(ApiPlanContext(bearerToken = None, connection = connection))
          .map(clubIds => resolved + (playerId -> clubIds))
      )
    }.map { playerClubIdsById =>
    resolvedPlayers.zipWithIndex.map { case (playerId, index) =>
      ...
      val clubId = playerClubIdsById.getOrElse(playerId, Vector.empty).headOption
      val clubShareAmount =
