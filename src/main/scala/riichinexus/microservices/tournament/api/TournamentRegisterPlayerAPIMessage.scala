    ...
    IO {
      ...
      module.transactionManager.inTransaction {
        ...
        if player.status != PlayerStatus.Active then
          throw IllegalArgumentException(s"Player ${playerIdValue.value} cannot enter tournament ${tournamentIdValue.value}")

        module.tournamentRepository.findById(tournamentIdValue).map { tournament =>
          TournamentSummaryView.fromDomain(module.tournamentRepository.save(tournament.registerPlayer(playerIdValue)))
        }
