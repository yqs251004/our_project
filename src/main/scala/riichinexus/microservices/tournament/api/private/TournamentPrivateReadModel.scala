...
import riichinexus.microservices.tournament.objects.`private`.{
...
/** TournamentPrivateReadModel 供后端服务执行赛事后端内部已读Model 流程，避免其它微服务直接访问内部表或领域模型。 */

private[tournament] object TournamentPrivateReadModel:
