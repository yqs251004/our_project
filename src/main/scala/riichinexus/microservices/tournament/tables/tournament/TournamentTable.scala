...
object TournamentTable:
  ...
  private[riichinexus] def findById(connection: Connection, id: TournamentId): Option[Tournament] =
  ...
  private[riichinexus] def findByNameAndOrganizer(
  ...
  private[riichinexus] def findByIds(connection: Connection, ids: Vector[TournamentId]): Vector[Tournament] =
  ...
  private[riichinexus] def findFiltered(
      connection: Connection,
  ...
  private[riichinexus] def findByClub(connection: Connection, clubId: ClubId): Vector[Tournament] =
  ...
  private def normalizeOnRead(connection: Connection, tournament: Tournament): Tournament =
    ...
    else save(connection, TournamentDefaults.ensureInitialStage(tournament))
