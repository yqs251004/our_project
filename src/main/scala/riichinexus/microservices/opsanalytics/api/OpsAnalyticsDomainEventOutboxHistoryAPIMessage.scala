...
final case class OpsAnalyticsDomainEventOutboxHistoryAPIMessage(
  ...
  override def plan(context: ApiPlanContext): IO[DomainEventOutboxHistoryView] =
    for
      ...
      _ <- IO(requireOpsAdmin(context, actor))
      history <- IO(buildHistory(context))
    yield history

  private def requireOpsAdmin(context: ApiPlanContext, actor: AccessPrincipal): Unit =
    context.support.opsAnalyticsModule.authorizationService
      .requirePermission(actor, Permission.ManageGlobalDictionary)

  private def buildHistory(context: ApiPlanContext): DomainEventOutboxHistoryView =
