...
final case class OptimisticConcurrencyException(
    ...
    actualVersion: Option[Int]
      ...
      s"Optimistic concurrency conflict for ${AggregateType.toString(aggregateType)}:$aggregateId. Expected version $expectedVersion, actual version ${actualVersion.getOrElse(-1)}"
    )
